...
\begin{abstract}
We present a data-analysis framework that combines random matrix
theory (RMT) correlation-threshold selection with a Monte Carlo
permutation-null test and a Dyson-Brownian-motion
eigenvalue-repulsion check, and apply it, in
...
value). A Monte Carlo permutation null --- shuffling each gene's values
across patients independently, 20 times per cohort --- shows both
thresholds are well beyond what dense-matrix universality alone could
produce: permuted TNBC data never sustains a GOE-like signature past
$\tau=0.25$ (20/20 replicates, exactly), four sweep steps below the
real $\tau^*=0.45$; permuted LumA data tops out at $\tau=0.15$ (19/20
replicates), two steps below the real $\tau^*=0.25$. Tracking the top eigenvalues of
...
not survive scrutiny and is not included in this paper's results; the
full analysis and control remain in
\texttt{scripts/04\_wavelet\_multiscale.py} and
\texttt{scripts/04b\_wavelet\_random\_order\_control.py} for the record.
A fair test would need either a real pseudo-time ordering (single-cell
data, Sec.~\ref{sec:roadmap}) or a synthetic positive control with a
planted multiscale signal, neither available within this paper's
scope.}
...
Two further lines were tested and came back as honest nulls, reported
as such rather than dropped: a wavelet multiscale decomposition (Line
5) whose headline result proved indistinguishable from a
random-ordering control, and TNBC-vs-LumA classification (part of the
ML line), which does not discriminate feature-selection methods. Two
