...
PC1-based approach. A validation-first collaboration should want all of
this reported exactly this way --- the real results, the honest nulls,
and the negative checks on the program's own prior assumptions alike.
The next concrete steps, in order of how directly they extend what is
already running, are: extending the (now two-cohort) Dyson-repulsion
scan to gene subsets other than the top-60-by-variance choice used
throughout; fixing the classifier-repulsion detector to rescale with
the empirical per-epoch step size before comparing early-window counts
across learning rates or architectures; pooling the single-cell result
across the other 8 TNBC patients in GSE176078 with an explicit
batch-effect check, and obtaining a genuinely non-PC1 pseudo-time
estimate (RNA velocity or a dedicated trajectory-inference tool) for a
third wavelet retest; extracting one chromosome's contact matrix from
GSE167150's already-processed \texttt{.hic} files (Line 3, still fully
unattempted) with \texttt{hicstraw}; and replacing the classification
...
\texttt{scripts/29\_new\_figures\_round3.py}, numbered in the order
...
learning rate/epoch budget, and \texttt{scripts/29\_new\_figures\_
round3.py} plots results from scripts 26--28); every number and figure
in this paper is produced directly by this code from the raw downloaded
files (two bulk TCGA-BRCA files, one single-cell sample), with no
