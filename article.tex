...
sample (894 Cancer Epithelial cells), as a corrected retest of the
wavelet line proposed, still finds no stronger ordering axis (PC1
variance share $8.6\%$, no better than the whole-sample $9.7\%$) and a
wavelet coarse/fine split statistically indistinguishable from a random
cell ordering; the same comparison against random orderings shows the
single-cell Dyson-repulsion pattern itself reflects growing sample size
rather than this specific ordering, clarifying rather than undermining
the bulk-cohort repulsion result.
...
originally proposed line left genuinely unattempted through several
earlier rounds --- 3D genomic structure via Hi-C --- is attempted here
too: the identical machinery, applied to one chromosome of a real TNBC
tumor's Hi-C contact map after the one standard preprocessing step Hi-C
data requires (observed/expected normalization), selects a real,
permutation-confirmed threshold and a bin-bin correlation matrix
showing the plaid pattern characteristic of chromatin compartments,
though independent validation of that compartment interpretation
remains for a follow-up round.
\end{abstract}
...
weight matrix is genuinely still changing --- makes a falsifiable
...
genuinely strong ordering axis.
...
panel) rather than an unstructured scatter of signs --- consistent
with, but not yet independently validated against, the classic
compartment interpretation. That validation (correlating the sign
pattern against an independent genomic feature such as gene density or
GC content, neither of which this round loads) is the concrete next
step before reading this eigenvector as biologically confirmed
compartments rather than a real but uninterpreted spectral feature.
...
Right: the top eigenvector's sign pattern along the chromosome,
forming contiguous blocks rather than scattered signs.}
\label{fig:hic}
\end{figure}

This is a first, single-chromosome, single-tumor result, not a
validated genome-wide compartment map: it establishes that the
identical spectral machinery this paper uses for gene expression
applies, without modification beyond the one standard O/E
preprocessing step, to real Hi-C contact data, and that the resulting
threshold is real by the same permutation-null standard applied
everywhere else in this paper. Extending to the other two real TNBC
tumors and the matched normal-tissue file already in GSE167150, other
chromosomes, and the independent compartment validation described
above are the direct next steps, stated concretely in
Sec.~\ref{sec:roadmap}.
...
Sec.~\ref{sec:singlecell-epithelial}) gives a genuinely different kind
of answer: restricting pseudo-time to the 894-cell Cancer Epithelial
lineage does not improve the ordering axis over the whole-sample PC1
($8.6\%$ vs.\ $9.7\%$ variance), leaves the wavelet retest a null
indistinguishable from a random ordering exactly as before, and --- most
usefully --- shows directly, for the first time, that the
Dyson-repulsion signature at single-cell resolution is a property of
growing sample size rather than of this specific ordering (random cell
orders reproduce it at comparable or higher rates). This is a
clarification of what ``time'' means in this diagnostic
(Definition~\ref{def:dyson-sde}), consistent with, not contradicted by,
the bulk-cohort reproducibility results, and it means the concrete
open item here is no longer ``restrict to one lineage'' (tried, and
insufficient on its own) but genuinely different pseudo-time inference
entirely (RNA velocity or a dedicated trajectory-inference tool).
...
\subsection{Line 3 revisited: beyond one chromosome, one tumor}
...
files among six 40\,kb--500\,kb-resolution files per condition,
270MB--2.5GB each, 6.2GB total for the series), and the concrete
single-file, single-chromosome first step that correction proposed has
now been run for real (Sec.~\ref{sec:results-hic}): chr18 of the
smallest real TNBC tumor file, observed/expected normalization
\citep{liebermanaiden2009}, and the identical RMT threshold machinery,
giving a real, permutation-confirmed $\tau^*=0.15$ and a plaid bin-bin
correlation pattern consistent with A/B compartment structure. The
direct next steps this result itself points to: (i) independently
validate the compartment interpretation of the top eigenvector's sign
pattern against genomic annotation (gene density or GC content along
chr18), rather than reading the plaid pattern and contiguous sign
blocks as compartments on visual resemblance alone; (ii) extend from
one chromosome to the rest of the genome and from one tumor to the
other two real TNBC tumors and the matched-normal file already in
GSE167150, directly testing whether the RMT-selected threshold and
compartment structure differ between tumor and normal tissue, the
comparison \citet{reyesgopar2025}'s own network measures (Jaccard
similarity between normal and TNBC networks, degree and Z-weighted
degree) were themselves designed for; (iii) a Monte-Carlo-permutation-
consistent but --- unlike the bulk and single-cell cohorts --- single-
sample threshold means this result cannot yet distinguish real
patient-to-patient variability from one tumor's own idiosyncrasy, the
direct reason for extending to the other two tumors before treating
$\tau^*=0.15$ as characteristic of TNBC Hi-C generally rather than of
this one sample.
...
directly, not just proposed, in two rounds now. First: the global PC1
of the single-cell data (Sec.~\ref{sec:results-singlecell}) carries
only $9.7\%$ of variance, no improvement over bulk-patient PC1's
$10.2\%$ (Sec.~\ref{sec:wavelets}), because this sample's real structure
is spread across many distinct cell-type axes rather than one dominant
gradient. Second, the corrected candidate that round proposed ---
pseudo-time restricted to the 894-cell Cancer Epithelial lineage alone
--- has itself now been tried (Sec.~\ref{sec:singlecell-epithelial})
and does not fix the problem either: its own PC1 explains only $8.6\%$
of variance, no better than the whole-sample figure, and the wavelet
retest on that ordering remains a null indistinguishable from a random
cell order. Restricting to a single lineage removes the categorical
T-cell-vs-everything-else split, but does not on its own supply a
strong continuous gradient. What remains is therefore not another restriction of the same
PC1-based approach, already tried twice, but a genuinely different
pseudo-time estimator --- RNA-velocity-style splicing information (not
available in this processed count matrix and would require
re-processing from raw reads) or a dedicated trajectory-inference tool
run on the same Cancer Epithelial cells --- before this line can be
...
entirely); (ii) the Dyson-trajectory diagnostic and the corrected,
single-lineage wavelet retest have both now been run
(Sec.~\ref{sec:singlecell-epithelial}) on the 894 Cancer Epithelial
cells in this same sample: the lineage-restricted PC1 does not improve
on the whole-sample ordering axis ($8.6\%$ vs.\ $9.7\%$ variance), the
wavelet coarse/fine split remains statistically indistinguishable from
a random cell ordering, and the Dyson-repulsion pattern itself proved
attributable to growing sample size rather than to this specific
ordering (a real-vs-random-order comparison not previously run at
single-cell resolution). What remains open on this front is a genuinely
different ordering signal --- real experimentally-derived pseudo-time
(e.g.\ RNA velocity or a published trajectory-inference tool run on
this same lineage) rather than a linear PC1 of any kind, since PC1
itself, restricted or not, has now been checked twice and found
insufficient both times. (iii) The global polynomial unfolding
...
Sec.~\ref{sec:limitations}) add genuinely new information here, since
...
cohorts (TNBC and, since this round, LumA) rather than asserted from
one, with a new quantitative corollary
(Corollary~\ref{cor:weak-repulsion}) explaining directly, from the same
SDE, which pairs fail to repel reliably and by how much in both
cohorts alike. It is also enough to surface three honest negative
results, reported as such rather than dropped: TNBC-vs-LumA
classification is too easy a benchmark to distinguish feature-selection
methods; a wavelet multiscale decomposition, tried as part of the
original six-line program, produced a headline result statistically
indistinguishable from a biologically meaningless random patient
ordering on two different ordering axes now (bulk PC1 and, since this
round, single-lineage single-cell pseudo-time alike), so it is reported
and excluded from the main results rather than kept as an unearned
positive finding; and the classifier's own weight-matrix eigenvalues,
tracked across SGD training (closing Roadmap item 6b) at two very
different learning rates now, repel far less reliably than the data's
do over a full run, but the repulsion present concentrates almost
entirely in the pre-convergence transient regardless of how long that
transient is made, a specific, quantified pattern rather than an
unexplained absence --- with an honest complication surfaced in the
second learning-rate regime: the fixed-epoch-window detector used
throughout this paper does not itself rescale with training speed, so
its raw counts should not be compared directly across learning rates
without that fix. The same pipeline also now runs on single-cell data
(Line 4, Sec.~\ref{sec:results-singlecell}) for the first time in this
program: a real, permutation-confirmed RMT threshold in a $q\ll1$
regime the bulk cohorts cannot reach, with spikes that split cleanly
into real cell-type axes (immune, myeloid, epithelial) and known
technical artifacts (ribosomal content, dissociation stress) once
checked against independent metadata --- and a direct, negative check,
now run twice, on the single-cell wavelet retest this Roadmap had
proposed: neither the sample's whole-population global PC1 nor its
PC1 restricted to the 894-cell Cancer Epithelial lineage alone carries
more signal than bulk PC1 did, so that line needs a genuinely different
pseudo-time estimator, not a different restriction of the same
PC1-based approach. The pipeline extends to a third data modality too:
3D genomic structure (Line 3, Sec.~\ref{sec:results-hic}), the one
originally proposed line left completely unattempted through several
earlier rounds, now has a real result --- one chromosome of a real TNBC
tumor's Hi-C map, observed/expected normalized following
\citet{liebermanaiden2009}, gives a real, permutation-confirmed RMT
threshold and a bin-bin correlation matrix with the plaid pattern
characteristic of A/B compartments, with the compartment reading itself
flagged as not yet independently validated rather than asserted on
visual resemblance alone. Every one of the six originally proposed
lines has now been attempted on real data at least once. A
validation-first collaboration should want all of this reported exactly
this way --- the real results, the honest nulls, and the negative
checks on the program's own prior assumptions alike. The next concrete
steps, in order of how directly they extend what is already running,
are: extending the (now two-cohort) Dyson-repulsion scan to gene
subsets other than the top-60-by-variance choice used throughout;
fixing the classifier-repulsion detector to rescale with the empirical
per-epoch step size before comparing early-window counts across
learning rates or architectures; pooling the single-cell result across
the other 8 TNBC patients in GSE176078 with an explicit batch-effect
check, and obtaining a genuinely non-PC1 pseudo-time estimate (RNA
velocity or a dedicated trajectory-inference tool) for a third wavelet
retest; validating the chr18 compartment call against real genomic
annotation and extending it to the other two real TNBC tumors and the
matched-normal file in GSE167150 (Line 3); and replacing the
classification benchmark with a within-TNBC or survival-outcome task
where a feature-selection advantage, if real, would actually be
visible.
...
\texttt{scripts/31\_hic\_figures.py}, numbered in the order
...
download helper, are named descriptively rather than fitting the main
...
23--24; \texttt{scripts/27\_luma\_dyson\_repulsion.py} and
\texttt{scripts/26\_singlecell\_epithelial\_pseudotime.py} are
...
round3.py} plots results from scripts 26--28, and
\texttt{scripts/31\_hic\_figures.py} plots results from script 30);
every number and figure in this paper is produced directly by this code
from the raw downloaded files (two bulk TCGA-BRCA files, one
single-cell sample, one Hi-C sample), with no
...
accession GSE176078 (GSM5354531). Hi-C data (sample GSM5098082,
``TNBC\_Tissue3'') are public and available from GEO accession
GSE167150. No proprietary or restricted-access data were used in this
paper.
