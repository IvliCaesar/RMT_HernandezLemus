...
variance across the full 1218-sample cohort, cohort-blind). Fewer than
2\% of candidate genes' correlation directions already exceed the
Marchenko--Pastur noise bulk and capture roughly half the total
variance in each cohort, and the leading such directions are
thematically coherent multi-gene programs (collagen/ECM, interferon,
keratin) rather than single-gene artifacts. The
...
do not overlap at all (0 of 10 shared); a random-ordering control,
however, shows the same zero overlap in 19 of 20 replicates, so this
result does not, on its own, confirm that the ordering axis carries
real biological signal, and we report it as an inconclusive negative
control rather than a positive finding. Tracking the top eigenvalues of
...
law at the $n^{-2/3}$ scale, so a handful of eigenvalues only marginally
...
\emph{CD300LG}, and \emph{ADIPOQ}. None of the ten top-degree hub genes
at either scale appear in the other scale's top ten.

This zero overlap is real but, on its own, weaker evidence than it
first looks like. We checked directly
(\texttt{scripts/04b\_wavelet\_random\_order\_control.py}) whether the
same zero-overlap result would appear under an ordering with no
biological meaning at all: rerunning the identical coarse/fine split
and RMT threshold selection 20 times, each time with an independent
uniformly random patient permutation instead of the PC1 ordering, gives
zero overlap in 19 of 20 replicates (mean $0.05/10$, range
$[0,1]$) --- statistically indistinguishable from the PC1-ordering
result. \textbf{This means the zero-overlap finding does not, by
itself, demonstrate that PC1 captures a real biological axis worth
decomposing along}; it is more likely a generic consequence of
splitting any sequence into a low-frequency approximation and a
high-frequency detail component, which are close to uncorrelated by
construction of the wavelet transform regardless of whether the
sequence order carries information. We report this as a real negative
control result, not as a failure to find something: it means the
version of \citet{konig2006}'s multiscale claim tested here (patients
ordered by a weak, $10.2\%$-of-variance proxy axis) is not verified,
and a fair test needs either (a) an ordering axis independently known
to carry real biological signal --- single-cell pseudo-time, per the
Line 4 roadmap below, rather than bulk-patient PC1 --- or (b) a
synthetic positive control with a planted true multiscale signal, to
first confirm the pipeline can detect one when it is actually there,
before trusting a positive result on real data where the ground truth
is unknown. Neither check was available within this paper's scope; the
coarse/fine hub tables above are kept as a description of what the
pipeline outputs, not as evidence for the multiscale hypothesis.
...
against LumA at $\approx\!98\%$ accuracy, well above the $78.3\%$
majority-class baseline (always predicting LumA, the larger of the two
classes) and confirmed by ROC-AUC $\approx\!0.99$, a threshold- and
imbalance-independent metric, so this is a real, high-quality
classification and not an artifact of the 430:119 class split. This is
an honest null
...
paper (RMT coexpression thresholding, spectral denoising/spiked
eigenvalues, wavelet multiscale decomposition, and a first
Dyson-repulsion diagnostic), two of those four came back as honest
nulls rather than positive findings (the wavelet multiscale separation
is indistinguishable from a random-ordering control, and TNBC-vs-LumA
classification does not discriminate feature-selection methods), and
two lines are not yet attempted with real data at all: 3D genomic
...
\subsection*{Line 5 revisited: a fair test of the multiscale claim}
The random-ordering control above shows the wavelet coarse/fine
separation as tested here does not confirm the multiscale hypothesis;
it neither confirms nor refutes it, since the test could not
distinguish a real weak signal (PC1, $10.2\%$ of variance) from no
signal at all. Two concrete fixes, either sufficient on its own: (a)
plant a known multiscale correlation signal in synthetic data (a
coarse-scale module and an independent fine-scale module, analogous to
\texttt{rmt\_threshold\_demo.py}'s planted-block design) and confirm
the pipeline recovers both distinctly before trusting it on real data;
or (b) rerun on GSE176078 single-cell data ordered by a real inferred
pseudo-time rather than bulk-patient PC1, where the ordering axis is
independently validated rather than a proxy.
...
correlation components identified by a Marchenko--Pastur threshold
rather than by eigenvector localization alone, and a first real-data
sighting of level repulsion among coexpression-matrix eigenvalues as
sample size grows. It is also enough to surface two honest negative
...
methods, and the wavelet coarse/fine hub-gene separation is
statistically indistinguishable from what a biologically meaningless
random patient ordering produces, so it does not yet confirm the
multiscale hypothesis it was meant to test. A validation-first
collaboration should want both reported exactly this way. The next
concrete steps, in order of how directly they extend what is already
running, are: reproducing the repulsion result across multiple random
orders with a confidence interval; applying the same RMT/wavelet
pipeline to GSE176078 single-cell data at the resolution Line 4
originally proposed, which also gives the wavelet test a real
pseudo-time ordering axis instead of the weak bulk-PC1 proxy used here
(Line 5, revisited); and replacing the classification benchmark with a
within-TNBC or survival-outcome task where a feature-selection
