...
LumA and to other gene subsets is the natural next step.
...
through \texttt{scripts/18\_repulsion\_reproducibility\_figure.py} in order (see
...
from one. It is also enough to surface two honest negative results,
...
already running, are: extending the (now reproducibility-checked)
Dyson-repulsion scan from TNBC to LumA and to other gene subsets;
applying the same RMT pipeline to GSE176078 single-cell data at the
resolution Line 4 originally proposed, which would also give a
revisited wavelet test a real pseudo-time ordering axis instead of the
weak bulk-PC1 proxy used here; and replacing the classification
...
\texttt{scripts/18\_repulsion\_reproducibility\_figure.py}, numbered in the order
