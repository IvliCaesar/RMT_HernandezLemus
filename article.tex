...
\\[4pt]
{\small $^{1}$ Facultad de Ciencias Empresariales, Universidad Panamericana, Ciudad de M\'exico, Mexico}\\
{\small $^{2}$ Computational Systems Biology and Integrative Genomics Laboratory,}\\
{\small Instituto Nacional de Medicina Gen\'omica (INMEGEN), Ciudad de M\'exico, Mexico}\\[6pt]
...
real biology and not sampling noise. We combine a classical RMT
correlation threshold, a Monte Carlo permutation null, and a
Dyson-Brownian-motion repulsion diagnostic into one pipeline and apply
it to real breast cancer data from The Cancer Genome Atlas (TCGA-BRCA),
comparing triple-negative (TNBC, $n=119$) and luminal A (LumA,
$n=430$) cohorts. The RMT threshold separates the two cohorts, a gap
confirmed by a permutation null; spiked eigenvalues beyond the
Marchenko--Pastur bulk correspond to biologically coherent gene
programs, not artifacts. Tracking eigenvalues as the sample grows
reveals Dyson-type level repulsion, reproducible across 20 random
orders in both cohorts, with a new corollary explaining which pairs
repel least reliably. The same pipeline extends, for the first time,
to two further data modalities: single-cell RNA-seq, recovering a real
threshold in a regime bulk data cannot reach and separating true
cell-type signal from technical artifacts; and Hi-C chromatin contact
data, recovering a compartment structure independently validated
against real GC content and gene density, replicated across three
triple-negative tumors and a matched normal sample. Three negative
results are reported, not dropped: a wavelet decomposition fails
identically under three unrelated cell-ordering axes, ruling out
ordering choice as the explanation; a classifier reaches near-ceiling
accuracy regardless of feature-selection method, showing the benchmark
cannot discriminate methods; and a trained classifier's own
weight-matrix eigenvalues repel only during the pre-convergence
training transient. All six research lines originally proposed for
this program have now been run on real data at least once, with code
and data fully reproducible. This validation-first approach, pairing
every positive finding with an explicit statistical null and reporting
negative results with equal rigor, offers a template for
computational-biology pipelines built on random matrix theory.
\end{abstract}

\section*{Author Summary}
Breast cancer is not one disease: triple-negative and luminal A tumors
differ enough that grouping them together can hide real biology. We
ask a mathematical question about this difference: when we measure how
strongly every pair of genes moves together across many patients, how
do we tell a real biological signal from statistical noise? We borrow
a tool from physics, random matrix theory, originally developed to
understand atomic-nucleus energy levels, that gives a principled
threshold for this, plus a permutation test confirming the threshold
is not an accident of the math. We apply this to real tumor data,
extend it for the first time to single cells and to the
three-dimensional folding of the genome, and check every positive
finding against independent evidence. We are equally careful to
report what did not work: a method for separating fast from slow
biological change, a machine-learning benchmark that turned out too
easy, and a training-dynamics diagnostic that only sees part of what
it was built to see. All of it, code, data, successes and failures
alike, is included so the next researcher can check, reuse, or correct
any of it.
...
analysis \citep{kcore2021}, multi-omic subtype-specific interaction
networks \citep{multiomic2022}, an information-theoretic account of how
coexpression tracks genomic (linear) distance between genes
\citep{gonzalezespinoza2021}, and, closest in kind to the network
characterization of Sec.~\ref{sec:networktopo} below, a direct study of
community structure --- Louvain modules and their functional,
transcriptional connectivity --- in the same kind of breast-cancer
coexpression networks \citep{deandajauregui2019networks}. The group's own network-inference
...
instead of relying on asymptotics alone. Second, \citet{aarts2024}
...
This paper makes three contributions. First, it states, as a single
explicit framework, how a static RMT threshold, a Monte Carlo
permutation null, and a Dyson-repulsion diagnostic relate to one
another --- the same underlying repulsion mechanism observed two
different ways, not two unrelated tools used side by side (Methods).
Second, it runs that framework, together with a machine-learning
recognition test, across three real, independent data modalities from
the same disease --- bulk RNA-seq (TCGA-BRCA, comparing TNBC against
luminal A breast cancer), single-cell RNA-seq, and Hi-C chromatin
contact data --- not on synthetic data, and not as a proposal: this is,
to our knowledge, the first application of a Dyson-repulsion diagnostic
to real gene-expression data at any resolution, and the first time a
single RMT pipeline has been run, unmodified beyond the one
data-specific preprocessing step each modality requires, across bulk,
single-cell, and 3D genomic data for the same cancer type. Third, it
reports every negative result the same way it reports every positive
one, without adjustment or omission: a wavelet multiscale method that
fails identically under three unrelated ordering axes, a classification
benchmark too easy to discriminate feature-selection methods, and a
classifier training diagnostic that sees only part of what it was built
to see. A validation-first pipeline is only as trustworthy as its
negative results are honest; this paper treats that discipline as a
result in itself, not as a shortfall to be hidden.
...
This is a precise, not merely qualitative, justification for
...
is visible when ``time'' is sample size instead, applied directly to a
real gene-correlation matrix and not a trained model.
\end{definition}
...
\qquad i=1,\dots,N,
\]
a different SDE from Definition~\ref{def:dyson-sde}'s (same
...
1962 result \citep{dyson1962}, now verified instead of only recorded.
...
eigenphases) confined to the unit circle instead of the real line,
...
spikes' top-loading genes form thematically coherent sets instead of
...
distance, following \citet{luo2007} exactly (own implementation in
\texttt{scripts/rmt\_threshold\_demo.py} and
\texttt{scripts/02\_rmt\_network\_analysis.py}, independent of, though
methodologically identical to, \texttt{RMTGeneNet}
\citep{gao2012} --- the maintained public tool implementing the same
method, now itself superseded by its authors' own successor,
\texttt{KINC}).
...
hierarchical clustering instead of left in arbitrary variance-rank
...
matched size (Erd\H{o}s--R\'enyi \citep{erdosrenyi1959}, same node/edge
count; the configuration model \citep{newmanstrogatzwatts2001}, same
node count \emph{and} the real network's own degree sequence) rather
than relying on the spectral diagnostics alone, following the same
network-science convention the Hern\'andez-Lemus group's own community-
structure work on breast-cancer coexpression networks already uses
\citep{deandajauregui2019networks}. Table~\ref{tab:networktopo} reports
clustering coefficient, giant component size, and Louvain-community
\citep{blondel2008} count/modularity \citep{newmangirvan2004} for both
cohorts.
...
Both cohorts show a real, unambiguous small-world \citep{wattsstrogatz1998}/modular signature:
...
bulk-patient PC1 provides; single-cell pseudo-time is the concrete
candidate (Sec.~\ref{sec:roadmap}), tested directly once real
single-cell data was in hand (Sec.~\ref{sec:results-singlecell} below).
The single-cell data's own PC1 (across the 7986 cells of patient
CID44971) carries only $9.7\%$ of variance --- essentially the same
order as bulk PC1 ($10.2\%$), not a large improvement. The reason is
visible directly in Sec.~\ref{sec:results-singlecell}'s own
spiked-eigenvalue result: this sample's real structure is spread
thinly across many distinct cell-type axes (immune, myeloid,
epithelial, stromal; 33 spikes sharing only $26.8\%$ of variance
combined) instead of concentrated in one dominant direction, so a
single global PC1 --- itself mostly a T-cell vs.\ everything-else split
in this mixed-tumor sample, not a smooth developmental gradient --- is
not the ``pseudo-time'' this line actually needs. A fair test requires
trajectory inference restricted to one continuously-varying lineage
(e.g.\ the 894 Cancer Epithelial cells alone, where a real
proliferation or differentiation gradient could exist, rather than the
whole 9-cell-type mixture), examined directly in
Sec.~\ref{sec:singlecell-epithelial}.
...
characteristic of level repulsion instead of free crossing. The
...
That last sentence was, until now, an intuition instead of a
...
This reproducibility check, run across 20 random orderings instead of
a single illustrative one, is evidence that the repulsion mechanism
Dyson formalized for randomly evolving Hermitian
matrices, and that \citet{aarts2024} find governing a classifier's
weight matrices during training, is also directly observable in the
spectrum of a real gene-coexpression correlation matrix as its sample
size grows --- a shared diagnostic for the data side and the model side
of a recognition pipeline built on this data (Line 6 of the wider
research program). Sec.~\ref{sec:results-dyson-luma} extends this
single-cohort, single-gene-subset check to LumA; the subsection after
that examines the other half of Line 6 directly, on real data: the
model's own weight-matrix dynamics.
...
spectral spike), in both real cohorts this paper analyzes.
Extending the same check to other gene subsets, beyond the top-60-by-
variance choice used throughout, remains open.

\subsection{Dyson dynamics of the classifier itself: a reproducible null}
...
signal, narrowing the failure down to a data problem instead of a
...
seeds at its own empirically measured convergence epoch instead of a
reused fixed cutoff.
...
both results together is therefore two-part instead of a single clean
...
needed before comparing raw counts across learning-rate regimes
directly. Figure~\ref{fig:classifier-longer-transient}
...
space instead of a sharper version of the same RMT signal. Overall
...
\subsection{Single-cell resolution (Line 4)}
\label{sec:results-singlecell}

Line 4 extends the bulk analysis to single-cell resolution using
GSE176078 \citep{tnbcsc2021}, a public single-cell dataset. We
downloaded patient CID44971 --- the largest of the series' 9
TNBC-labeled samples (7986 cells, 29733 genes, real cell-type
annotation from \citealp{tnbcsc2021}) --- and applied the identical RMT
machinery used throughout this paper (Sec.~\ref{sec:math},
\ref{sec:spiked}, \ref{sec:tau-sweep}) unchanged: no new methodology,
only new data. A single patient, rather than a pooled multi-patient
cohort, is used deliberately to avoid a real confound the bulk analysis
does not face --- cross-patient batch effects that could otherwise
masquerade as spiked eigenvalues; extending this to a pooled cohort is
left as a limitation.
...
($q=12.61$ TNBC, $3.49$ LumA), with a correspondingly tight
...
biological axes (cell types) instead of concentrated in a few.
...
$\tau\ge0.125$, with $0.10$ sitting exactly at the crossover instead of
...
reflecting a poorly-conditioned tail fit rather than large
...
scheme, instead of one global polynomial shared with the much sparser
bulk cohorts, is the concrete fix needed here.
...
Line 4 reproduces the bulk cohorts' central finding (real,
permutation-confirmed RMT threshold structure, spiked eigenvalues
beyond the Marchenko--Pastur edge) in a completely different regime
($q\ll1$ instead of $q\gg1$), while also surfacing a single-cell-
specific caveat the bulk data never raised (technical
dissociation/ribosomal spikes indistinguishable from biological ones
without independent metadata). It also settles the question
Sec.~\ref{sec:wavelets} left open about single-cell pseudo-time: this
sample's own (whole-sample) PC1 does not provide a stronger ordering
axis, so single-lineage trajectory inference is needed instead of
single-cell data of any kind, examined directly in
Sec.~\ref{sec:singlecell-epithelial}. One patient, one subtype, is the
honest scope of this result; extending it across the other 8 TNBC
patients in the series (pooled, with an explicit batch-effect check
this single-patient analysis did not need) is left as a limitation.

\subsection{Single-lineage pseudo-time (Line 5)}
\label{sec:singlecell-epithelial}

Pseudo-time restricted to a single, continuously varying lineage,
rather than the whole 9-cell-type mixture, is directly available in the
same sample: 894 Cancer Epithelial cells
(Sec.~\ref{sec:results-singlecell}), the largest non-immune population
and the natural candidate for a proliferation or differentiation
gradient. Running the identical PC1-ordering, Dyson-trajectory, and
wavelet-decomposition pipeline used elsewhere in this paper
(Sec.~\ref{sec:epithelial-methods}) on this subset alone gives three
specific answers.
...
turn this population into a strong continuous gradient; whatever
developmental or proliferative structure this lineage has is evidently
not concentrated in a single dominant linear direction either.

Second, the wavelet retest itself (Line 5) remains null on
...
retest replaces PC1 itself with a non-linear estimator built
...
above, now for a third time on a different ordering:
...
diagnostic's repulsion signature tracks growing sample size instead of
...
categorical-mixture PC1, one single-lineage PC1, and now one
...
\subsection{3D genomic structure (Line 3)}
\label{sec:results-hic}

Line 3 targets the one originally proposed research line not yet run
on real Hi-C data. \citet{reyesgopar2025} provides a public,
patient-cohort-scale TNBC Hi-C dataset (GSE167150); the same group's
information-theoretic account of spatial regulatory organization in
breast-cancer transcriptomics \citep{deandajauregui2019spatial} is the
closest existing bridge, in its own prior work, between coexpression
and genomic spatial structure, though built from expression data alone
rather than Hi-C contact maps directly. We run the identical RMT
threshold/spiked-eigenvalue machinery used throughout this paper on
real Hi-C data: GSM5098082 (``TNBC\_Tissue3''), the smallest of
GSE167150's three real TNBC tumor \texttt{.hic} files, chromosome 18 at
100kb resolution (Sec.~\ref{sec:hic-methods}). One chromosome, the
smallest of the three real TNBC tumors, is a deliberately narrow first
step.
...
compartments, but neither is proof: we check the interpretation
directly, against two real, independent genomic
...
such instead of dropped: the wavelet method's real-data application
...
resolution (Line 4) is run on GSE176078 \citep{tnbcsc2021}, one TNBC
patient (7986 cells), using the identical RMT machinery
(Sec.~\ref{sec:results-singlecell}), in a regime where $q=p/n$ inverts
relative to the bulk cohorts, with a real permutation-confirmed
threshold and a new, single-cell-specific caveat (ribosomal/
...
had to face. 3D genomic structure (Line 3, Hi-C) is run on GSE167150
(TNBC Hi-C with matched contralateral healthy tissue,
\citealp{reyesgopar2025}), one chromosome (chr18) of the smallest of
its three real TNBC tumor \texttt{.hic} files, observed/expected
normalization following \citet{liebermanaiden2009}, and the identical
RMT threshold machinery (Sec.~\ref{sec:results-hic}) --- a real,
permutation-confirmed threshold and a bin-bin
...
The Dyson-repulsion result (Sec.~\ref{sec:results-dyson}) rests, in its
single-order form, on one cohort, one 60-gene subset, and one random
patient-inclusion order; the multi-order reproducibility check
(Fig.~\ref{fig:repro}) resolves the one open question this leaves
(whether $\lambda_2$--$\lambda_3$ repels at all) in the affirmative.
Corollary~\ref{cor:weak-repulsion} gives the one lingering data-side
anomaly ($\lambda_1$--$\lambda_2$'s unreliability) a quantitative
account directly from the same SDE. The single-cohort limitation is
closed too: the identical check on LumA
...
spiked-eigenvalue count (Table~\ref{tab:spiked}). What remains open is
narrower: extending the multi-order check to gene subsets other than
the top-60-by-variance choice used throughout.
...
$\rho=-0.481$ against PC1 --- a different ordering, not PC1
...
at single-cell resolution again tracks growing sample size instead of
...
closes with a specific, mechanistic finding rather than a flat null:
weight-matrix repulsion is real but confined to the pre-convergence
training transient, a 12-fold difference between the early and late
windows (\texttt{scripts/22\_classifier\_dyson\_early\_late.py}). The
direct follow-up question --- does a longer transient produce a longer
detectable-repulsion window? --- is answered directly in
Sec.~\ref{sec:results-dyson-classifier-longer}: at a $5\times$ smaller
learning rate the transient lengthens by $\sim\!11\times$ (not the
$5\times$ a linear relationship would predict), the post-convergence
null gets even cleaner ($0/140$ vs.\ $3/140$), and the qualitative
claim holds, but the raw pre-convergence detection count actually
falls ($11/140$ vs.\ $36/140$) because the closest-approach/reopening
criterion's fixed 5-epoch window does not rescale with the slower
per-epoch dynamics --- a real limitation of the detector, honestly
surfaced instead of smoothed over, with a concrete methodological fix:
a window sized to the empirical per-epoch step, not a fixed epoch
count.
...
All six originally proposed lines have now been run on real data at
least once (Sec.~\ref{sec:results-hic}, \ref{sec:results-singlecell},
\ref{sec:results-dyson-classifier}); each subsection below states the
concrete next step each result points to.

\subsection{Line 3: beyond one chromosome}
\citet{reyesgopar2025} --- Hern\'andez-Lemus's own group --- provides a
public, patient-cohort-scale TNBC Hi-C dataset (GEO accession
GSE167150, three real TNBC tumor \texttt{.hic} files and a
matched-normal file among six 40\,kb--500\,kb-resolution files per
condition, 270MB--2.5GB each, 6.2GB total for the series). Every
concrete next step this dataset makes possible has now been run for
real (Sec.~\ref{sec:results-hic}): chr18 of all three real TNBC
...
comparison those measures were themselves designed for and which is
not attempted here.

\subsection{Line 5: three orderings, one answer}
...
so what remains is not a method fix but a data fix. This is checked
directly, on three unrelated ordering axes. First: the global PC1 of
...
cell-type axes instead of one dominant gradient. Second, pseudo-time
...
\citep{haghverdi2016} on that same lineage --- a non-linear
...
require re-processing from raw reads) or a real time-course --- or
...
expression-derived pseudo-time estimator of the same
kind as the two already tried --- a single global projection, linear or
not --- is not a promising next step on its own. A mechanistically
different expression-derived method is a partial exception worth
naming precisely rather than folding into that same dismissal:
\texttt{Slingshot} \citep{street2018slingshot} fits principal curves
through pre-identified cluster centers rather than projecting all cells
onto one axis at once, using cluster adjacency (not a single global
direction) to build the ordering --- different enough in mechanism from
both PC1 and diffusion pseudotime that its failure would not be
guaranteed by this section's result, though its success is not
guaranteed either, and it still shares the same fundamental limit as
the two methods already tried: all three read pseudo-time off gene
expression itself, not off an independent physical clock.
...
whether any spike loads across layers instead of within a single one
...
\subsection{Line 4: beyond one patient}
Line 4 is run on real data (Sec.~\ref{sec:results-singlecell}):
...
ordering but a different \emph{kind} of signal ---
...
\subsection{Line 6b: beyond one small classifier}
Line 6b is run on real data (Sec.~\ref{sec:results-dyson-classifier}):
a small MLP's weight-matrix Gram eigenvalues do not repel as reliably
as the data-side correlation-matrix eigenvalues over a full 300-epoch
training run, but the repulsion that is present concentrates almost
entirely in the pre-convergence transient (epochs 0--49), a 12-fold
difference against the post-convergence window. The direct follow-up
question --- does a longer transient produce a longer
detectable-repulsion window? --- is answered directly
...
The comparison sketched in the Introduction --- RMT spectral thresholding
...
pattern instead of an unexplained absence, with one further honest
...
The same pipeline runs, for the first time, on two further data
modalities beyond bulk expression. On single-cell RNA-seq (Line 4), it
gives a real, permutation-confirmed threshold in a $q\ll1$ regime the
...
independent metadata. On Hi-C contact data (Line 3), it gives a real,
permutation-confirmed threshold and a bin-bin
...
connected component size, and Louvain-community
\citep{blondel2008} count and modularity score
\citep{newmangirvan2004} (\texttt{networkx}, \citealp{networkx2008};
Louvain instead of greedy-modularity maximization, for tractability on
LumA's $\sim$146K-edge network), and compared each against the mean
over 3 replicates of two random-graph nulls of matched size: an
Erd\H{o}s--R\'enyi \citep{erdosrenyi1959} $G(n,p)$ graph (same node and
edge count, no other constraint) and a configuration-model graph
\citep{newmanstrogatzwatts2001} built by fast stub-matching (same node
count \emph{and} the real network's own degree sequence, testing for
structure beyond degree heterogeneity alone).
...
directly from script 30 instead of duplicating them, and
...
proprietary or restricted-access data were used in this paper; all
accession numbers needed to reproduce every download are also listed
in Methods.

\section*{Code availability statement}
All analysis code (\texttt{scripts/01\_build\_cohorts.py} through
\texttt{scripts/35\_hic\_validation\_figures.py}, Methods) is included
with this submission as Supporting Information and is also publicly
available without restriction at
\url{https://github.com/IvliCaesar/RMT_HernandezLemus}.

\section*{Ethics statement}
Not applicable. This study uses only publicly available, de-identified
data (TCGA-BRCA, GSE176078, GSE167150); no new human-subjects data
were collected and no institutional review was required.
...
J.C.G.: Conceptualization, Data curation, Formal analysis, Investigation,
Methodology, Software, Validation, Visualization, Writing -- original
draft. E.H.L.: Conceptualization, Domain expertise, Supervision,
Writing -- review \& editing. Both authors read and approved the final
manuscript.
...
comparison (Introduction, Future directions).
...
\newblock \emph{Journal of Statistical Mechanics: Theory and Experiment} \textbf{2008}, P10008 (2008).
...
\bibitem[de Anda-J\'auregui et al.(2019a)]{deandajauregui2019networks}
G.~de~Anda-J\'auregui, S.~A.~Alc\'ala-Corona, J.~Espinal-Enr\'iquez, E.~Hern\'andez-Lemus,
\newblock ``Functional and transcriptional connectivity of communities in breast cancer co-expression networks,''
\newblock \emph{Applied Network Science} \textbf{4}, 22 (2019).

\bibitem[de Anda-J\'auregui et al.(2019b)]{deandajauregui2019spatial}
G.~de~Anda-J\'auregui, J.~Espinal-Enr\'iquez, E.~Hern\'andez-Lemus,
\newblock ``Spatial Organization of the Gene Regulatory Program: An Information Theoretical Approach to Breast Cancer Transcriptomics,''
\newblock \emph{Entropy} \textbf{21}(2), 195 (2019).
...
\bibitem[Erd\H{o}s \& R\'enyi(1959)]{erdosrenyi1959}
P.~Erd\H{o}s, A.~R\'enyi,
\newblock ``On random graphs I,''
\newblock \emph{Publicationes Mathematicae Debrecen} \textbf{6}, 290--297 (1959).
...
\bibitem[Gonz\'alez-Espinoza et al.(2021)]{gonzalezespinoza2021}
A.~Gonz\'alez-Espinoza, J.~M.~Zamora-Fuentes, E.~Hern\'andez-Lemus, J.~Espinal-Enr\'iquez,
\newblock ``Gene Co-Expression in Breast Cancer: A Matter of Distance,''
\newblock \emph{Frontiers in Oncology} \textbf{11}, 726493 (2021).
...
H.~K.~Kim, K.~H.~Park, Y.~Kim, S.~E.~Park, H.~S.~Lee, S.~W.~Lim, et al.,
...
E.~Lieberman-Aiden, N.~L.~van~Berkum, L.~Williams, M.~Imakaev, T.~Ragoczy, A.~Telling, et al.,
...
F.~Luo, Y.~Yang, J.~Zhong, H.~Gao, L.~Khan, D.~K.~Thompson, et al.,
...
A.~A.~Margolin, I.~Nemenman, K.~Basso, C.~Wiggins, G.~Stolovitzky, R.~Dalla~Favera, et al.,
...
\bibitem[Newman \& Girvan(2004)]{newmangirvan2004}
...
K.~Street, D.~Risso, R.~B.~Fletcher, D.~Das, J.~Ngai, N.~Yosef, E.~Purdom, S.~Dudoit,
\newblock ``Slingshot: cell lineage and pseudotime inference for single-cell transcriptomics,''
\newblock \emph{BMC Genomics} \textbf{19}, 477 (2018).
...
\bibitem[Watts \& Strogatz(1998)]{wattsstrogatz1998}
D.~J.~Watts, S.~H.~Strogatz,
\newblock ``Collective dynamics of `small-world' networks,''
\newblock \emph{Nature} \textbf{393}, 440--442 (1998).

\bibitem[Wu et al.(2021)]{tnbcsc2021}
S.~Z.~Wu, G.~Al-Eryani, D.~Roden, S.~Junankar, K.~Harvey, A.~Andersson, et al.,
...
\texttt{results/luma\_rmt.csv} (also S1 Table and S2 Table below).
...
\paragraph{S1 Table.}
\textbf{Full TNBC RMT threshold sweep.} Every threshold value tested
(not only $\tau^*$), with edge count, network density, and both KS
distances at each; reproduced in Table~\ref{tab:tnbc-full} above and
available as \texttt{results/tnbc\_rmt.csv} in the public code
repository (Code availability statement).

\paragraph{S2 Table.}
\textbf{Full LumA RMT threshold sweep.} Same format as S1 Table,
reproduced in Table~\ref{tab:luma-full} above and available as
\texttt{results/luma\_rmt.csv}.

\paragraph{S1 Data.}
\textbf{Complete analysis code and intermediate results.} All numbered
scripts (\texttt{scripts/01\_build\_cohorts.py} through
\texttt{scripts/35\_hic\_validation\_figures.py}), every intermediate
result table referenced throughout the text
(\texttt{results/*.csv}), and every figure-generation script, publicly
available without restriction at
\url{https://github.com/IvliCaesar/RMT_HernandezLemus} (Code
